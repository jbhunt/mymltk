...
- ViT (visual transformer) for image classification
...
## Image generation
I've also implemented some basic/fundamental generative techniques for image generation like a variational autoencoder (VAE). The figure below illustrates my implementation of a VAE trained on the UCI hand-written digits dataset.

![](/data/img/image_generation.png)

From top-to-bottom, rows show target images, the VAE's reconstruction of the target images, and new images generated by sampling the VAE.

Here's the same model fit to the CIFAR-10 dataset:

![](/data/img/image_generation_cifar.png)
